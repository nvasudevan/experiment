...
run_randomcfg() {
    ...
    do
        ...
        do
            ...
            amb=$(echo $out | grep -o 'ambiguous string' | uniq)
	        rm $gcfg
            ((cnt+=1))
            ...
            then 
                ...
                echo "$randomsize - $g,yes,$ambstr,$(echo $ambstr | wc -w)" | tee -a $result
                continue
            fi
            amb=$(echo $out | grep -o 'unambiguous\!')
            [ "$amb" == "unambiguous!" ] && echo "$randomsize - $g,unambiguous,," | tee -a $result  && continue
    ...
    do
        ...
        do
            ...
            then 
                ((ambcnt+=1))
                ambstr=$(echo $out | awk -F': ' '{print $2}')
                echo "$g.$i,yes,$ambstr,$(echo $ambstr | wc -w)" | tee -a $result
                continue
            fi
            amb=$(echo $out | grep -o 'unambiguous\!')
            [ "$amb" == "unambiguous!" ] && echo "$g.$i,unambiguous,," | tee -a $result  && continue
    ...
    do
          ...
          do
             ...
             then
                 ((ambcnt+=1))
                 ambstr=$(echo $out | awk -F': ' '{print $2}')
                 echo "$g.0_$n,yes,$ambstr,$(echo $ambstr | wc -w)" | tee -a $result
                 continue
             fi
             amb=$(echo $out | grep -o 'unambiguous\!')
             [ "$amb" == "unambiguous!" ] && echo "$g.0_$n,unambiguous,," | tee -a $result  && continue
    ...
    do    
        ...
        do
            ...
            then 
                ((ambcnt+=1))
                ambstr=$(echo $out | awk -F': ' '{print $2}')
                echo "$boltzsize - $g,yes,$ambstr,$(echo $ambstr | wc -w)" | tee -a $result
                continue
            fi
            amb=$(echo $out | grep -o 'unambiguous\!')
            [ "$amb" == "unambiguous!" ] && echo "$boltzsize - $g,unambiguous,," | tee -a $result  && continue
...
run_test() {
    ...
    do
        ...
        then
            ((ambcnt+=1))
            ambstr=$(echo $out | awk -F': ' '{print $2}')
            echo "$g,yes,$ambstr,$(echo $ambstr | wc -w)" | tee -a $result
            continue
        fi
        ambstr=$(echo $out | grep -o 'unambiguous\!')
        [ "$ambstr" == "unambiguous!" ] && echo "$g,unambiguous,," | tee -a $result  && continue
        echo "$g,,," | tee -a $result
    done
    print_summary $ambcnt $cnt
}
