...
run_lang() {
    ...
    do
        ...
        do
            ...
            CFG_GRAMMAR_FILE="${GRAMMAR_DIR}/acla/${grammar}.${i}.cfg"
            [ ! -d ${GRAMMAR_DIR}/acla ] && mkdir -p ${GRAMMAR_DIR}/acla    
...
run_mutlang() {
    ...
    do
        ...
        do
            ...
            CFG_GRAMMAR_FILE="${MUTATED_LANGUAGES}/cfg/${grammar}/${grammar}.0_${n}.cfg"
            [ ! -d ${MUTATED_LANGUAGES}/cfg/${grammar} ] && mkdir -p ${MUTATED_LANGUAGES}/cfg/${grammar}    
            cat ${ACC_GRAMMAR_FILE} | egrep -v "^\s*;|^%nodefault|^%token " > ${CFG_GRAMMAR_FILE}
            token_list="`grep '%token' ${ACC_GRAMMAR_FILE} | sed -e 's/%token //' | tr -d ';,'`"
            for token in $token_list
            do
            ...
            sentence="`timeout ${TIME} ${CMD} -a ${CFG_GRAMMAR_FILE} | egrep -o 'unambiguous\!|ambiguous string' | uniq`"
            [ "${sentence}" == "ambiguous string" ] && echo "${grammar}.0_${n},yes" | tee -a ${RESULT}  && continue
            [ "${sentence}" == "unambiguous!" ] && echo "${grammar}.0_${n},no" | tee -a ${RESULT}  && continue
